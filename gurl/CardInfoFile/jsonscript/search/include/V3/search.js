 function recostSearch_error(error){
setnote("حدث خطأ");
console.log('Error',error);
 }

function recostSearch_done(recosts) {
...
function goToSearch(){
        if (window.matchMedia('(min-width:576px)').matches) {
			...
			$("#boxSearch").focus();
const UserSidebarOffcanvas = document.getElementById('UserSidebar');
UserSidebarOffcanvas.addEventListener('hidden.bs.offcanvas', event => {
 window.scrollTo({top: 0,behavior: 'smooth'});
})	
        } else {
			$('#UserSidebar').offcanvas('hide');
            $('#collapseSearchPhone').collapse('show');
			$("#boxSearch2").focus();
const UserSidebarOffcanvas = document.getElementById('UserSidebar');
UserSidebarOffcanvas.addEventListener('hidden.bs.offcanvas', event => {
 window.scrollTo({top: 0,behavior: 'smooth'});
