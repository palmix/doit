...
function setUserPost(recosts,UserAccountIds,UserNames,UserGenders){
...
setNomPostUser(UserAccountId,recosts);
setPropertiesUser(AccountStatus);
setProfilePostsView(UserAccountId);
  
setUserPost(recosts,UserAccountId,UserName,UserGender);
...
function setNomPostUser(UserAccountId,recosts) {
var nom = 0;
UserAccountId = UserAccountId.toString();
$.each(recosts.sheets[0].data[0].rowData,function( i, item ) {
var UserAccount_Id = item.values[44].formattedValue;//معرف حساب المستخدم
UserAccount_Id = UserAccount_Id.toString();
if(UserAccount_Id == UserAccountId){
nom++;
}
$("#UserPosts").html(nom);
...
function setProfilePostsView(UserAccountId){
var elem = $("#ViewMyPosts");
var blogStats = new Firebase("https://geoarabic1.firebaseio.com/pages/posts/");
...
var c = Object.keys(data)[i];
var x = Object.values(data)[i];

if(x.userid == UserAccountId){
view +=  x.value;
}
