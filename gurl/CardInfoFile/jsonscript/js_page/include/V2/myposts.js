...
setMyposts_Error(error);

    }).done(function(recosts) {
    setMyposts(recosts,UserAccountIds,UserNames,UserGenders);
    });
 }
 
 function setMyposts_Error(error){
...
function setMyposts(recosts,UserAccountIds,UserNames,UserGenders){
...
var cardse1 = '<div class="row row-cols-1 row-cols-md-3 g-4 mt-2">';
var cardse2 = '<div class="row row-cols-1 row-cols-md-3 g-4 mt-2">';
var cards1 = [cardse];
var cards2 = [cardse1];
var cards3 = [cardse2];
var cards1num=0;
var post0 = 0;// غير منشور
var post1 = 0;// تم نشره
var post2 = 0;// منشور بوضع مجهول
...
if(getuserId == UserAccountId){
if (Published === "0"){
	
post0 += 1;

function GetList3(num) {
...
var btn_favoriteID = "P" + p;
 ...
 num.push('<a href="'+PostUrlInSite_Answer+'" data-post-id="'+PostIdInSite_Answer+'" data-card-href="'+cardURL+'" style="height:'+Img_H+'px;" class="'+showModalClassImg+' dataimg rounded-top" onclick="'+showModalOnClickImg+'" data-id="'+ResponseNumber+'" title="'+ScientificName_Answer+'"><img src="'+ImgCover_Answer+'" width="'+Img_W+'" height="'+Img_H+'" loading="lazy" class="card-img-top lazyload" alt="'+CommonName_Answer+'"></a>');
...
var btn_favoriteID = "P" + p;
 ...
 num.push('<div class="card-body text-center p-0 rounded-bottom">');
 num.push('<a href="'+PostUrlInSite_Answer+'" data-post-id="'+PostIdInSite_Answer+'" data-card-href="'+cardURL+'" data-id="'+ResponseNumber+'" onclick="'+showModalOnClick+'" title="'+CommonName_Answer_En+'" class="list-group-item list-group-item-action border-0 '+showModalClass+'">'+CommonName_Answer+'</a>');
num.push('<p class="card-text">هذه المشاركة منشورة بالوضع المجهول</p>');
 num.push('</div>');
 num.push('</div>');
 num.push('</div>');
}
...
if (post1 <= 0){
$("#PageCardName").addClass('mt-4 bg-success bg-gradient text-white mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركاتي المنشورة</h5>');
  $("#PageCardName").after('<div class="alert alert-light d-flex align-items-center rounded-0 rounded-bottom" role="alert"><i class="fas fa-rss mx-2 fs-4"></i><div>لا يوجد لك أي مشاركات  منشورة</div></div>');
}else{
$("#PageCardName").addClass('mt-4 bg-success bg-gradient text-white mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركاتي المنشورة</h5>');
}
...
if (post0 <= 0){
$("#PageCardNameunpublished").addClass('mt-4 bg-danger bg-gradient text-white mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركات غير منشورة</h5>');
  $("#PageCardNameunpublished").after('<div class="alert alert-light d-flex align-items-center rounded-0 rounded-bottom" role="alert"><i class="fas fa-rss mx-2 fs-4"></i><div>لا يوجد لك أي مشاركات غير منشورة</div></div>');
}else{
$("#PageCardNameunpublished").addClass('mt-4 bg-danger bg-gradient text-white mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركات غير منشورة</h5>');
}

if (post2 <= 0){
$("#PageCardNameAnonymous").addClass('mt-4 bg-warning bg-gradient text-dark mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركاتي بالوضع المجهول</h5>');
  $("#PageCardNameAnonymous").after('<div class="alert alert-light d-flex align-items-center rounded-0 rounded-bottom" role="alert"><i class="fas fa-rss mx-2 fs-4"></i><div>لا يوجد لك أي مشاركات بالوضع المجهول</div></div>');
}else{
$("#PageCardNameAnonymous").addClass('mt-4 bg-warning bg-gradient text-dark mb-0 p-2 rounded-top text-center typeicone').html('<h5>مشاركاتي بالوضع المجهول</h5>');
}
...
document.getElementById('cardslistallUser1').innerHTML = '';
document.getElementById('cardslistallUser2').innerHTML = '';
...
document.getElementById('cardslistallUser1').innerHTML +=  cards2.join('');
document.getElementById('cardslistallUser2').innerHTML +=  cards3.join('');
...
//console.log(jsondata)
    ...
    var getIdCard_ = $(this).attr("data-id");
    var IdCard_ = getIdCard_.replace('ID1000000', '');
    var IdCards_ = parseInt(IdCard_)
    var IdCard_ = IdCards_;

var new_item = recosts.sheets[0].data[0].rowData.reverse()[IdCards_ + 2];
var testgetcard = new_item.values[1].formattedValue;// ID



if(getIdCard_ != testgetcard){
new_item = recosts.sheets[0].data[0].rowData.reverse()[IdCards_ + 2];
}
...
var table = [''];
        if (Published == 0) {
table = ['<div class="alert alert-danger" role="alert"> هذه المشاركة غير منشورة! </div>'];
        } else {
	
if(Name_publishing == 'no'){
table = ['<div class="alert alert-secondary" role="alert"> لقد قمت بنشر هذه المشاركة بشكل مجهول </div>'];
}
		}
